...
//  Use only one loading stack for all steps
//  Support later stage requesting 
//  Fix TSM_REF base and interface constrain not working bug
...
private:
	...
	RuntimeType* LoadFields(std::unique_ptr<RuntimeType> type, Type* typeTemplate)
	{
		...
		for (auto typeId : tt->Fields)
		{
			...
			{
				assert(std::any_of(_loadingTypes.begin(), _loadingTypes.end(),
